...
/// MPartners fullscreen HTML interstitial. Same load → present → dismiss
/// pattern as `EBInterstitialAd`. Single-use: after dismiss, `isReady` flips
/// back to false and the host must call `load()` again.
final class MPartnersInterstitialController: UIViewController {
    ...
    private var interstitial: EBMPartnersInterstitialAd?

    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .systemGroupedBackground
        title = "MPartners Interstitial"
    ...
    @objc private func handleLoad() {
        ...
        let ad = EBMPartnersInterstitialAd(adUnitId: AdUnitIds.MPartners.interstitial)
