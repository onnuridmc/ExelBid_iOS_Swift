...
/// MPartners 320x50 banner. No autoRefresh — the host must tap Load to refresh.
@interface MPartnersBannerController ()
...
@property (nonatomic, strong, nullable) EBMPartnersBannerAd *bannerAd;
@end

@implementation MPartnersBannerController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.systemGroupedBackgroundColor;
    self.title = @"MPartners Banner";
    ...
    [actions addArranged:[EBPrimaryButton filled:@"Load MPartners banner" target:self action:@selector(handleLoad)]];
...
- (void)handleLoad {
    EBMPartnersBannerAd *ad = self.bannerAd ?: [self makeBannerAd];
    [ad stop];
    [self setState:EBStatusBadgeStateLoading text:@"loading…"];
    [ad load];
}

- (EBMPartnersBannerAd *)makeBannerAd {
    EBMPartnersBannerAd *banner =
        [[EBMPartnersBannerAd alloc] initWithAdUnitId:AdUnitIdsMPartners.banner
