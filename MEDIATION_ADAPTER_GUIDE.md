...
ExelBid는 주요 광고망용 미디에이션 어댑터를 별도 패키지로 제공합니다.

- **AdMob** (`ExelBidMediationAdMob`) — 배너 / 전면 / 네이티브 / 비디오
- **FAN** (`ExelBidMediationFAN`) — 배너 / 전면 / 네이티브 / 비디오
- **AdFit** (`ExelBidMediationAdFit`) — 배너 / 전면 / 네이티브
  (AdFit은 전체화면 비디오 포맷이 없어 비디오 어댑터는 제외)

하지만 다음과 같은 경우 **어댑터를 직접 만들어 쓸 수 있습니다.**

- 제공 패키지에 없는 광고망(예: 사내 광고 서버, 신규 네트워크)을 붙이고 싶을 때
- 제공 어댑터의 동작이 마음에 들지 않아 **포크해서 커스터마이즈**하고 싶을 때
  (예: AdMob 배너의 adaptive size 정책 변경)
- 제공 어댑터가 **다루지 않는 포맷**이 필요할 때 (예: AdFit 비디오)

어댑터는 공개 프로토콜만 구현하면 되므로, 제공 패키지와 똑같은 방식으로
누구나 만들 수 있습니다. 이 문서는 **제공 어댑터 패키지
(`ExelBid_iOS_Mediation_Adapter`)를 템플릿 삼아** 직접 어댑터를 만드는
방법을 설명합니다.

> 미디에이션의 전체 동작 원리와 호스트 측 사용법은
> [`README.md`](./README.md) §미디에이션 을 참고하세요.
